...
Q2 ) How has the spiciness (scoville rating) of the sauces used in "Hot Ones" episodes changed over different seasons? Is there any correlation between the distributions and the lengeth of the seasons?
...
-   For the second question, we plan on extracting the average scoville data of the sauces that appeared in each of the seasons, for all of the seasons, and draw box plots (or violin plot.). We will compare compare it to a bar plot which represents the entire length of the season as the seasons length (number of episodes) were not the same for each season. By doing this we will see if seasons that had a higher average number of scolville data had shorter seasons (number of episodes), and seasons with a lower overall average scoville number had more episodes within a season.
